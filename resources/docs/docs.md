...
The `home-routes` handler in the `time-align.routes.home` namespace
...
The `home-page` function will in turn call the `time-align.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/time_align/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `time-align.handler` namespace:
...
Request middleware functions are located under the `time-align.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `time-align.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `time-align.db.core` in some other namespace.
